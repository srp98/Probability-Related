...
def get_log_probabilities(file_list):
    """
    Computes log-frequencies for each word that occurs in the files in file_list.
    ...
    Output
    ------
    A dict whose keys are words, and whose values are the log of the smoothed estimate of the fraction of files the key
    occurred in.

    Hint
    ----
    The data structure defaultdict from collections python library will be useful to you here, as will the get_counts()
    helper above.
    """
    word_counts = get_counts(file_list)
    n_files = len(file_list)
    n_categories = 2
    log_prob = defaultdict(lambda: -np.log(n_files + n_categories))     # Laplace Smoothing (L=1), default dictionary
    for word in word_counts:
        log_prob[word] = np.log(word_counts[word] + 1) - np.log(n_files + n_categories)     # Laplace Smoothing with L=1
...
def learn_distributions(file_lists_by_category):
    ...
    Input
    -----
    A two-element list. The first element is a list of spam files, and the second element is a list of ham (non-spam)
    files.
    ...
    log_probabilities_by_category : A list whose first element is a smoothed estimate for log P(y=w_j|c=spam) (as a dict
                                    just as in get_log_probabilities above), and whose second element is the same for
                                    c=ham.

    log_prior_by_category : A list of estimates for the log-probabilities for each class: [est. for log P(c=spam),
    est. for log P(c=ham)]
    """
...
def classify_email(email_filename, log_probabilities_by_category, log_prior_by_category):
    ...
    Output
    ------
    One of the labels in names [Spam or Ham].
    """
    email_words = set(util.get_words_in_file(email_filename))
    n_categories = len(log_prior_by_category)

    # Get union of all words encountered during training
    all_words = []
...
def classify_emails(spam_files, ham_files, test_files):
    log_probabilities_by_category, log_prior = learn_distributions([spam_files, ham_files])
    estimated_labels = []
    for test_file in test_files:
        estimated_label = classify_email(test_file, log_probabilities_by_category, log_prior)
...
def main():
    ...
    # Here, columns and rows are indexed by 0 = 'spam' and 1 = 'ham' rows correspond to true label, columns correspond
    # to guessed label
    ...
    for filename in (util.get_files_in_folder(testing_folder)):
        ...
        # Uncomment line to see which files classifier gets right/wrong:
