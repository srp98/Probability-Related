...
#        0 <= y <= GRID_HEIGHT - 1,
#        action is one of {'left', 'right', 'up', 'down', 'stay'}.
# Note that <action> refers to the *previous* action taken so that we arrived at (x, y). In particular, it is NOT
# guaranteed to be the next action to be taken by the robot.
#
...
class Distribution(dict):
    """
    The Distribution class extend the Python dictionary such that each key's value should correspond to the probability
    of the key.
    For example, We can create a random variable X that takes on value 'spam' with probability .7 and 'eggs' with
    probability .3:
    ...
    Methods:
    renormalize(): scales all the probabilities so that they sum to 1
    get_mode(): returns an item with the highest probability, breaking ties arbitrarily
...
# Functions specifying the robot model (e.g., listing all possible hidden and observed states, initial distribution,
...
def observation_model(state):
    ...
    for x_new in range(x - radius, x + radius + 1):
        for y_new in range(y - radius, y + radius + 1):
            if 0 <= x_new <= GRID_WIDTH - 1 and 0 <= y_new <= GRID_HEIGHT - 1:
