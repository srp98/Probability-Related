...
class playback_positions(tk.Tk):
    ...
    def __init__(self, true_positions, observed_positions, estimated_positions, estimated_marginals, *args, **kwargs):
        tk.Tk.__init__(self, *args, **kwargs)
        self.wm_title('Robot Demo')
        self.canvas = tk.Canvas(self,
                                width=GRID_WIDTH * CELL_WIDTH,
                                height=(GRID_HEIGHT * CELL_HEIGHT + PADDING) * 3,
                                borderwidth=0,
        ...
        for column in range(GRID_WIDTH):
            for row in range(GRID_HEIGHT):
                ...
                x2 = x1 + CELL_WIDTH
                y2 = y1 + CELL_HEIGHT
                self.rect_top[row, column] = self.canvas.create_rectangle(x1, y1, x2, y2,
                                                                          fill='black',
                                                                          tags='rect_top',
                                                                          outline='gray11')
                ...
                y2 = y1 + CELL_HEIGHT
                self.rect_middle[row, column] = self.canvas.create_rectangle(x1, y1, x2, y2,
                                                                             fill='black',
                                                                             tags='rect_middle',
                                                                             outline='gray11')

                y1 = row * CELL_HEIGHT + 2 * (GRID_HEIGHT * CELL_HEIGHT + PADDING)
                y2 = y1 + CELL_HEIGHT
                self.rect_bottom[row, column] = self.canvas.create_rectangle(x1, y1, x2, y2,
                                                                             fill='black',
                                                                             tags='rect_bottom',
                                                                             outline='gray11')

        self.robot_top = self.canvas.create_oval(0, 0, 0, 0, fill='gray80')
        self.robot_top_arrow = self.canvas.create_polygon(0, 0, 0, 0, 0, 0, 0, 0)
        ...
        self.robot_middle_arrow = self.canvas.create_polygon(0, 0, 0, 0, 0, 0, 0, 0)
        self.canvas.create_text((GRID_WIDTH * CELL_WIDTH / 2.,
        ...
        self.robot_bottom_arrow = self.canvas.create_polygon(0, 0, 0, 0, 0, 0, 0, 0)
        self.canvas.create_text((GRID_WIDTH * CELL_WIDTH / 2.,
        ...
        self.redraw(true_positions, observed_positions, estimated_positions, estimated_marginals, 200)

    def get_trapezoid_coords(self, x1, x2, y1, y2, direction):
        # for generating heat map polygons 1/3 and 2/3
        vertices = list()
        y13 = y1 / 3. + y2 * 2. / 3.
        y23 = y1 * 2. / 3. + y2 / 3.
        x13 = x1 / 3. + x2 * 2. / 3.
        x23 = x1 * 2. / 3. + x2 / 3.
        if direction == 'up':
            vertices.append([(x1, y1), (x2, y1), (x23, y13), (x13, y13)])
        elif direction == 'down':
            vertices.append([(x1, y2), (x2, y2), (x23, y23), (x13, y23)])
        elif direction == 'left':
            vertices.append([(x1, y1), (x1, y2), (x13, y23), (x13, y13)])
        elif direction == 'right':
            vertices.append([(x2, y1), (x2, y2), (x23, y23), (x23, y13)])
        elif direction == 'stay':
            vertices.append([(x13, y13), (x23, y13), (x23, y23), (x13, y23)])
    ...
    def color_heatmap_grid(self, marginals):
        ...
        for x in range(GRID_WIDTH):
            for y in range(GRID_HEIGHT):
                ...
                self.canvas.itemconfigure(self.rect_bottom[y, x], fill='#%02x%02x%02x' % (int(color), 0, 0))
